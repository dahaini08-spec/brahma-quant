...
if [ -z "$PID" ]; then
  ...
  if [ -n "$NEW_PID" ]; then
    ...
    # 推送通知
    openclaw message send -t "73295708:thread:01a07628-0405-7e85-a34b-e68cd029dfc6" --channel jarvis --message "⚠️ Supercronic已重启 PID=$NEW_PID" >/dev/null 2>&1 &
  else
    echo "$(date -u '+%Y-%m-%d %H:%M UTC') [watchdog] supercronic重启失败！"
    openclaw message send -t "73295708:thread:01a07628-0405-7e85-a34b-e68cd029dfc6" --channel jarvis --message "🔴 Supercronic重启失败！手动检查" >/dev/null 2>&1 &
  fi
fi
