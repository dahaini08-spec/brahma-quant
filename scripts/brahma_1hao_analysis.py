...
def fmt_breakdown(bd: dict) -> str:
    ...
    sections = {
        ...
        "AI/ML层": ["ML+在线贝叶斯+滑点", "LSTM+NLP情绪", "HMM乘数", "研究增强层",
                    "L7_Kronos", "L7_Kronos_v2", "M09_维度权重"],
        "宏观层": ["L2+贝叶斯+宏观", "宏观+事件", "清算流追踪"],
        "时段/体制层": ["时段权重", "N03时段奖励", "N08_牛市RSI中性", "N10_全覆盖奖励",
                       "N15_分层仓位", "N16_ATR体制", "_regime", "_regime_mult",
                       "N04周末降权", "N14_体制切换时机", "N14_熊转边界",
                       "N15_仓位倍数", "T04体制最优", "TF共振奖励", "TF权重调整",
                       "_regime_v4_key"],
        "波动率层": ["HAR-RV波动率", "VolProfile密度"],
        "协同奖励层": ["N01协同奖励", "N06持仓建议", "N08_RSI强化", "N08_RSI强化_v2",
                      "N12_BB上沿", "N12_BB下沿",
                      "CVD订单流", "Hurst体制验证"],
        "猎手层": ["WICK_HUNTER_LONG", "WICK_HUNTER_LONG_WEAK", "WICK_HUNTER_SHORT",
                  "P0_压缩最优窗口", "P0_长压缩泡压弱"],
        "量价协同层": ["P1_量价有效", "P1_量价齐升",
                      "P2_RSI中性机构入场",
                      "P3_BEAR_SHORT_RSI最佳", "P3_BEAR_SHORT_RSI陷阱",
                      "Phase2c_RSI中性偏强", "Phase2c_量能×RSI协同"],
        "FIX修正层": ["FIX1_假牛市", "FIX2_CHOP追空"],
    }
...
def run_analysis(symbol: str, direction: str = 'LONG', compact: bool = False) -> str:
    ...
    try:
        ...
        if _fc_data and _fc_data.get('status') == 'ok':
            ...
            # [P0修复 2026-08-29 苏摩111] 直接调用get_features()实时计算，不读缓存_anchor_summary
            _htf = _fc_data.get('htf_anchor', {})
            try:
                from brahma_brain.weekly_monthly_anchor import get_anchor as _wma_get
            ...
            if _htf and (_htf.get('htf_bias') or _htf.get('_anchor_summary')):
                _htf_bias_v = _htf.get('htf_bias', 'NEUTRAL')
                _htf_res_v  = _htf.get('htf_resonance', 0.0)
                _htf_add_v  = int(_htf.get('score_addon', 0) or 0)
                _w52h = _htf.get('52w_high', 0)
                _w52l = _htf.get('52w_low', 0)
                w3_pos = _htf.get('weekly_position', 0.5)
                _fc_lines.append(f"  【HTF周月线锚定】{_htf_add_v:+d} ({_htf_bias_v} 共振={_htf_res_v:.2f})")
                if _w52h > 0 and _w52l > 0:
                    _fc_lines.append(f"  52W区间: ${_w52l:,.0f}~${_w52h:,.0f} | 周线位置:{w3_pos*100:.1f}% | HTF共振:{_htf_res_v:.2f}")
