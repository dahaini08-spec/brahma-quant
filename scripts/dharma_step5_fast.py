"""dharma_step5_fast.py — Step5: 只跑2个关键配置"""
...
def run_bt(label, exit_fn, rr=2.0, min_score=100):
...
def main():
    t0=time.time()
    R={}
    print("🔥 Step5: ms=100+RR=2.0 关键配置")
    print("="*70)
    R['ms100_natural']=run_bt('ms=100 natural RR=2.0',exit_natural)
    R['ms100_trailing']=run_bt('ms=100 trailing RR=2.0',exit_trailing)
    elapsed=time.time()-t0
    print(f"\n耗时: {elapsed:.1f}s")
    print(f"\n{'配置':<30} {'n':>6} {'WR':>6} {'IC':>8} {'netEV':>8} {'Sharpe':>7} {'WFstab':>7}")
    print("-"*75)
    for k,r in R.items():
        if r: print(f"{r['label']:<30} {r['n']:>6} {r['wr']:>5.1f}% {r['ic']:>+8.4f} {r['pnl_net']:>+7.3f}% {r['sharpe']:>7.2f} {r['wf_stab']:>6.1%}")
    with open(OUT_DIR/'dharma_step5_result.json','w') as f:
