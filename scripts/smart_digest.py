...
def push_digest():
    ...
    try:
        ...
        thread_id = getattr(cfg, 'JARVIS_THREAD_ID', '019fd70a-0942-72b1-aeb9-1bd4fc11b30d')
        to = f'{user_id}:t:{thread_id}' if user_id and thread_id else f'{user_id}:thread:019fd70a-0942-72b1-aeb9-1bd4fc11b30d'
    except Exception:
        to = '73295708:thread:019fd70a-0942-72b1-aeb9-1bd4fc11b30d'  # [BUG-3 修复] 外层异常时精确地址即SSOT
    import subprocess
    result = subprocess.run(
        ['openclaw', 'message', 'send', '--channel', 'jarvis', '-t', to, '--message', digest],
