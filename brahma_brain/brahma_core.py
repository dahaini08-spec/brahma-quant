...
def confluence_score(ms: dict, smc: dict, signal_dir: str,
    ...
    if signal_dir == 'LONG' and ob.get('nearest_bull_ob'):
        ...
        s2 += 5 if d < 0.5 else (3 if d < 1.5 else (1 if d < 3.0 else 0))  # [P1-A] 3%内也有少量得分
    if signal_dir == 'SHORT' and ob.get('nearest_bear_ob'):
        d = abs(ob['nearest_bear_ob']['dist_pct'])
        s2 += 5 if d < 0.5 else (3 if d < 1.5 else (1 if d < 3.0 else 0))
    # [P1-A upgrade 2026-06-17] 4H OB 双层确认奖励（MTF共振）
    ...
    try:
        ...
        if _cur_px > 0:
            ...
            if signal_dir == 'LONG':
                _ld_adj = -_ld_adj  # 反转：做多时下方清算密集是负面
            if _ld_adj != 0 and _ld.get('confidence', 0) >= 0.3:
                s7 = max(0, min(15, s7 + _ld_adj))
                print(f'[s7-LiqDens] {_sym} {signal_dir}: {_ld_adj:+d} | bias={_ld["liq_bias"]} conf={_ld["confidence"]}')
    except Exception:
        pass
...
# [CLEANED 2026-06-11] from kelly_allocator import compute as _kelly_compute
    ...
    # ── 死穴二次物理封锁（哲学原则三加固）─────────────────────────────────────
    # BEAR_TREND_LONG(WR=45%) / BULL_TREND_SHORT(WR=47.7%) 永久封禁
    ...
    if 'BEAR_TREND' in _regime_str and _dir_check == 'LONG':
        _valid = False
        cf['breakdown']['死穴封禁'] = 'BEAR_TREND_LONG WR=45%永久封禁(铁证n=3322)'
    elif 'BULL_TREND' in _regime_str and _dir_check == 'SHORT':
        _valid = False
        cf['breakdown']['死穴封禁'] = 'BULL_TREND_SHORT WR=47.7%永久封禁(铁证n=4999)'
    ...
    try:
        ...
        if _etg_active and not _etg_exempt:
            # 额外结构惩罚 -8分（叠加N22b的-10分，总计-18分）
            _etg_penalty = -8
            _score_raw = round(_score_raw + _etg_penalty, 1)
            cf['total'] = _score_raw
            cf['etg_penalty'] = (
                f'EarlyTrendGate[v25.4]: {_etg_regime}×{_etg_dir} '
                f'逆势 RSI={_etg_rsi1h:.0f} {_etg_penalty:+d}分 → {_score_raw:.0f}'
            )
            print(f'[EarlyTrendGate] {_sym} {_etg_regime}×{_etg_dir}: {_etg_penalty:+d}分 RSI={_etg_rsi1h:.0f}')
    ...
    # ══════════════════════════════════════════════════════════════
    try:
        _mth = int(__import__('datetime').datetime.utcnow().month)
    ...
    try:
        ...
        if len(_kl15m) >= 60:
            ...
            if _s23 != 0:
                ...
                _result['s23_p_up'] = _s23_meta.get('p_up', 0.5)
                _unlock_tag = f" 🔓UNLOCK:{_unlock['regime']}" if _unlock.get('unlocked') else ''
                print(f'[s23-Kronos] {_sym_t} {_dir_t}: {_s23:+d}'
                      f' | p_up={_s23_meta.get("p_up","?"):.2f}'
                      f' | src={_s23_meta.get("source","?")}{_unlock_tag}')
