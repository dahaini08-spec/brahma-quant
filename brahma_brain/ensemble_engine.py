...
def get_ensemble_score(symbol: str, direction: str, brahma_result: dict = None) -> dict:
    ...
    if not vec:
        return {
            ...
            'true_alpha': [],
            'n_dims': 13,
            'error': 'failed to extract 13 features',
        }
...
def _extract_12vec_from_result(symbol: str, brahma_result: dict = None) -> dict:
    ...
    # Phase 5: 跨市场alpha（134标的FR差异）
    cross_market_val = 0.0
    try:
    ...
    return {
        ...
        'cross_market': round(cross_market_val, 4),
    }
...
def _normalize_dim(dim_name: str, value: float, direction: str) -> float:
    ...
    elif dim_name == 'cross_market':
        # 跨市场alpha已方向感知[-1,+1]，直接返回
