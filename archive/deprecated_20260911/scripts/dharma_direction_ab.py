"""dharma_direction_ab.py — 方向A(ms=110/120) + 方向D(maker成本) 同时验证"""
...
def run_bt(label, rr=2.0, min_score=100, cost=0.0008, slip=0.0005):
    ...
    print(f"  硬标准: {pass_count}/4 {'🟢 全通过!' if pass_count==4 else '⚠️'}")
    ...
    return {'label':label,'n':n,'wr':round(wr,1),'ic':ic,'pnl_net':round(pnl_n,3),
            ...
            'wf_pass':pf,'wf_fail':ff,'hard_pass':pass_count}

def main():
    t0=time.time(); R={}

    print("="*70)
    print("🔥 方向A: ms=110/120 + RR=2.0（提高门槛→更高WR）")
    print("="*70)
    R['A_ms100']=run_bt('基线 ms=100',rr=2.0,min_score=100)
    R['A_ms105']=run_bt('ms=105',rr=2.0,min_score=105)
    R['A_ms110']=run_bt('ms=110',rr=2.0,min_score=110)
    R['A_ms115']=run_bt('ms=115',rr=2.0,min_score=115)
    R['A_ms120']=run_bt('ms=120',rr=2.0,min_score=120)

    print("\n"+"="*70)
    print("🔥 方向A2: ms=110 + RR=2.5/3.0（极品信号+更宽TP）")
    print("="*70)
    R['A_ms110_rr25']=run_bt('ms=110 RR=2.5',rr=2.5,min_score=110)
    R['A_ms110_rr30']=run_bt('ms=110 RR=3.0',rr=3.0,min_score=110)

    print("\n"+"="*70)
    print("🔥 方向D: maker成本假设（0.13%→0.10%→0.08%）")
    print("="*70)
    # maker cost = 0.0002 (2bp) + 0.0003 (3bp slippage) = 0.0005 total per side
    # 原版: taker 0.0008 + 0.0005 = 0.0013 per side
    # maker: 0.0002 + 0.0003 = 0.0005 per side
    # 但回测里cost是单边，slip是单边，合计0.0013双边
    # 改为maker: cost=0.0002, slip=0.0003
    R['D_maker_ms100']=run_bt('maker ms=100',rr=2.0,min_score=100,cost=0.0002,slip=0.0003)
    R['D_maker_ms110']=run_bt('maker ms=110',rr=2.0,min_score=110,cost=0.0002,slip=0.0003)

    print("\n"+"="*70)
    print("🔥 方向A+D: ms=110 + maker成本 + RR=2.0/2.5")
    print("="*70)
    R['AD_ms110_maker']=run_bt('ms=110+maker RR=2.0',rr=2.0,min_score=110,cost=0.0002,slip=0.0003)
    R['AD_ms110_maker_rr25']=run_bt('ms=110+maker RR=2.5',rr=2.5,min_score=110,cost=0.0002,slip=0.0003)
    ...
    print(f"\n耗时: {elapsed:.1f}s")
    with open(OUT_DIR/'dharma_direction_ab_result.json','w') as f:
