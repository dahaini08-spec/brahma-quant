"""dharma_rr_sweep.py — RR=2.5/3.0 + ms=110 + maker 突破WF≥60%"""
...
def run_bt(label, rr=2.0, min_score=110, cost=0.0002, slip=0.0005):
    ...
    return {'label':label,'n':n,'wr':round(wr,1),'ic':ic,'pnl_net':round(pnl_n,3),
            ...
            'wf_pass':pf,'wf_fail':ff,'hard_pass':pc,'exit_dist':dict(ed)}

def main():
    t0=time.time(); R={}
    print("="*70)
    print("🔥 RR扫描: ms=110 + maker(0.10%) + RR=2.0/2.5/3.0/3.5")
    print("="*70)
    for rr in [2.0, 2.5, 3.0, 3.5]:
        R[f'rr{rr}']=run_bt(f'ms=110 maker RR={rr}',rr=rr,min_score=110)

    print("\n"+"="*70)
    print("🔥 ms=105 + maker + RR=2.0/2.5（更多信号）")
    print("="*70)
    R['ms105_rr20']=run_bt('ms=105 maker RR=2.0',rr=2.0,min_score=105)
    R['ms105_rr25']=run_bt('ms=105 maker RR=2.5',rr=2.5,min_score=105)
